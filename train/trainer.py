...
class Trainer:
    ...
    def run(
        ...
        if distributed_option.distributed:
            if trainer_options.sharded_ddp:
                ...
                dp_model = torch.nn.parallel.DistributedDataParallel(
                    ...
                    find_unused_parameters=True,#trainer_options.unused_parameters,
                )
        ...
        for iepoch in range(1, 2):
            logging.info(f"{iepoch} / 1 : PAC identity initialization...")
            stats = cls.train_one_epoch_pac(
                model=dp_model,
                ...
                pac=pac,
                optimizers_pac=optimizers_pac,
            )
            logging.info(f"{stats.item()}")
    ...
    def train_one_epoch_pac(
        ...
        for iiter, (utt_id, batch) in enumerate(iterator, 1):
            ...
            with autocast(scaler is not None):
                retval = model(**batch, identity=True)
                # Note(kamo):
                # Supporting two patterns for the returned value from the model
                #   a. dict type
                loss_mse, stats, weight = retval
                optim_idx = None

                if ngpu > 1 or distributed:
                    # Apply weighted averaging for loss and stats
                    loss_mse = (loss_mse * weight.type(loss_mse.dtype)).sum()
                ...
                if distributed:
                    ...
                    loss_mse *= torch.distributed.get_world_size()

                loss_mse /= accum_grad

            if scaler is not None:
                loss_pac = loss_mse
                scaler.scale(loss_pac).backward(inputs=pac_params)
            else:
                loss_pac = loss_mse
                loss_pac.backward(inputs=pac_params)

            if iiter % accum_grad == 0:
                if scaler is not None:
                    for iopt, optimizer in enumerate(optimizers_pac):
        ...
        for iiter, (utt_id, batch) in enumerate(
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time"):
                    ...
                    #   b. tuple or list type
                        ...
                        if pac: (loss, loss_mse, loss_ctc_pac), stats, weight = retval
                        else: loss, stats, weight = retval
                        optim_idx = None
                ...
                if ngpu > 1 or distributed:
                    ...
                    if pac:
                        loss_mse = (loss_mse * weight.type(loss_mse.dtype)).sum()
                        loss_ctc_pac = (loss_ctc_pac * weight.type(loss_ctc_pac.dtype)).sum()
                ...
                if distributed:
                    ...
                    if pac:
                        loss_mse *= torch.distributed.get_world_size()
                        loss_ctc_pac *= torch.distributed.get_world_size()

                loss /= accum_grad
                if pac:
                    loss_mse /= accum_grad
                    loss_ctc_pac /= accum_grad
            ...
            with reporter.measure_time("backward_time"):
                if scaler is not None:
                    ...
                    #scaler.scale(loss).backward(inputs=model_params, retain_graph=True)
                    scaler.scale(loss).backward(retain_graph=True)
                    if pac:
                        loss_pac = loss_mse - 0.0001*loss_ctc_pac
                        scaler.scale(loss_pac).backward()
                else:
