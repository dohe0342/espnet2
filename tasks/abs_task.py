...
class AbsTask(ABC):
    ...
    def build_optimizers(
        ...
        for n , p in model.named_parameters():
            if 'pac' not in n and p.requires_grad:
                params.append(p)
        ...
        if args.sharded_ddp:
            ...
            #optim = optim_class(model.parameters(), **args.optim_conf) ## original code
    ...
    def build_optimizers_pac(
        ...
        params = []
        for n , p in model.named_parameters():
            if 'pac' in n and p.requires_grad:
                params.append(p)
                print(n)
        ...
        if args.sharded_ddp:
            ...
            #optim = optim_class(params, **args.optim_conf)
            logging.info('custom optimizer loading...')
            #optim = apex.optimizers.FusedAdam(params, lr=0.001)
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        pac = True
        if pac: 
            optimizers_pac = cls.build_optimizers_pac(args, model=model)
